...
4. **Module-scope clients must never throw (Vercel build failure, 2026-06-12).** `createClient(url || '', …)` at module scope killed a production deploy: supabase-js throws on an empty url, the module is evaluated while prerendering `/_not-found`, and that build step didn't see the env. Local builds always pass (.env.local exists) — so this class of bug is invisible until Vercel. Rule: module-scope SDK inits use syntactically-valid placeholders + a `configured()` gate; real failures surface at call time, never at import time.
5. **Callback plumbing + visible outcomes (TextDrip sync bug, 2026-06-11).** When a child component produces a RESULT PAYLOAD and hands it up via callback, the parent must accept it — wiring `onDone={() => doThing()}` (arg-dropping closure) instead of `onDone={doThing}` silently discarded the sync results and triggered a duplicate scan; the import then happened invisibly minutes later. Rules: (a) payload-producing callbacks are passed by reference, and the handler validates the payload shape (so a stray click event can't no-op); (b) `await` the handler so spinners cover the WHOLE operation; (c) render the operation's outcome INLINE next to the button that triggered it (persistent text, not only a transient toast) — "nothing visibly happened" must be impossible.
