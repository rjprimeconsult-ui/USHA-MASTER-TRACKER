/**
 * GET  /api/benepath/config — load the caller's Benepath config; generates a
 *                             webhook token on first call if none exists.
 * POST /api/benepath/config — save { defaultStage, regenerateToken? }.
 *
 * Auth: bearer token → getUser (anon client).
 * Storage: service-role for profiles (benepath_webhook_token) + user_kv
 *          (benepath_config_v1).
 *
 * Unlike Ringy there is no disposition→stage mapping — Benepath posts brand-new
 * leads, so only a single default stage is configurable.
 *
...
export async function GET(req) {
  try {
    ...
    const [token, cfgRow] = await Promise.all([
      ...
      admin.from('user_kv').select('value').eq('user_id', userId).eq('key', 'benepath_config_v1').maybeSingle(),
    ]);
    ...
    console.error('[benepath/config GET] error:', e);
...
export async function POST(req) {
  try {
    ...
    const { defaultStage, regenerateToken } = body || {};
    ...
    console.log(`[benepath/config POST] user=${userId} regenerated=${!!regenerateToken}`);

    const origin = originFromRequest(req);
    return jsonResponse(200, buildResponseShape(origin, token, updatedCfg));
  } catch (e) {
    console.error('[benepath/config POST] error:', e);
    return jsonResponse(500, { error: `Server error: ${e?.message || String(e)}` });
  }
}
