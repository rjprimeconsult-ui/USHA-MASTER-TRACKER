...
**Current data model (unchanged by this rollout):** each blast row is `{ id, platform, runDate, campaignOrTag, contacts, sendTime, rangeStart, rangeEnd, notes, numbersUsed, source }`. Auto-vs-manual is discriminated by the **`id` prefix**, not the `source` string: an id starting with `bc:` is an **auto (Ringy) counter row**; anything else is a manual/TextDrip row. (Do **not** rely on `source === 'log'` — manual rows default `source` to `'auto'`; use the `bc:` prefix, which is what the parent already does.) Persistence lives in the parent (`LeadTracker.jsx`) via `onEdit` / `onAdd` / `onDelete` props; `BlastsView` is presentational. **Do not change storage:** keep `rangeStart` / `rangeEnd` as two stored strings and `campaignOrTag` as the stored string. Everything below is UI over that same shape.

**How edits reach storage — the two paths behind `onEdit(id, form)` (must-read for F2/F3):**
- **Manual rows** (`id` not `bc:*`) → updated in place in the `blast_log_v1` client store.
- **Auto rows** (`id` starts with `bc:`) → written to the **`blast_counters` table**, and because `tag` is part of the row key, **editing the Campaign/Tag rekeys the row** (`bc:run_date:platform:tag`). This is pre-existing behavior (the current pencil form does it); F only surfaces it inline. Do **not** reimplement the counter write in F — route inline edits through the *same* `onEdit(id, form)`.
- **⚠️ Count-safety rule (mandatory):** the parent's `onEdit` **reconstructs the whole record from the `form` object** it receives — including `contacts = parseInt(form.contacts) || 0`. Therefore an inline editor **must call `onEdit(id, fullForm)` with a COMPLETE form** — the row's existing values for every field (`platform, runDate, campaignOrTag, contacts, sendTime, rangeStart, rangeEnd, notes`) merged with the one edited field — **never a single-field patch.** Sending a partial patch would make `contacts` come back `undefined → 0` and silently zero a blast count. This is the single most important correctness rule in Rollout F.
...
- **Separator rule (important):** only split on a separator that is **space-surrounded** — ` → `, ` - `, ` – `, or ` to ` (the word `to` only when flanked by spaces). This prevents a hyphenated single date like `03-26-2026` from being mis-split. Prefer showing ` → ` as the canonical separator.
...
- These **three** cells become **click-to-edit in place** in the Blast Log table: click the cell → it becomes its editor (the F1 field for Lead Range, the F3 combobox for Campaign/Tag, a plain text input for Notes) → **Enter or blur commits**; **Esc cancels**. On commit, call `onEdit(id, fullForm)` per the **Count-safety rule above** — merge the row's existing field values with the one edited field; **never send a single-field patch** (it would zero `contacts`).
...
- **Verification per rollout** (in the running preview, both themes):
  ...
  - F — type a combined lead range and confirm it splits into `rangeStart`/`rangeEnd` and renders right (test a single date and a hyphenated `03-26-2026` value — the latter must NOT split); inline-edit Lead Range, Campaign/Tag, and Notes and confirm each saves. **Count-safety checks:** after each inline edit, the row's **Contacts count is unchanged** (proves the full-form merge, not a partial patch); on an **auto (Ringy) row** (`bc:` id), edit the Campaign/Tag and confirm the row **rekeys cleanly — no duplicate row, no orphan, totals unchanged** (and if the new tag collides with another same-day/same-platform row, the existing error-rollback fires rather than corrupting data). The Campaign/Tag dropdown lists previously-used tags and accepts a brand-new one; auto Ringy rows still lock Date & Send Time.
